...
if ($_SERVER["REQUEST_METHOD"] == "POST" && SESSION::estaLogueado('USER')) {
    ...
    if ($validador->validaDNI($candidato_convocatoria->DNI) == true) {
    } else {
        $num_errores++;
    }

    if ($validador->validaNombre($candidato_convocatoria->nombre, 100, 1) == true) {
    } else {
        $num_errores++;
    }

    if ($validador->validaNombre($candidato_convocatoria->apellido1, 100, 1) == true) {
    } else {
        $num_errores++;
    }

    if ($validador->validaContraseña($candidato_convocatoria->contrasena) == true) {
    } else {
        $num_errores++;
    }

    if ($validador->validaCorreo($candidato_convocatoria->correo) == true) {
    } else {
        $num_errores++;
    }

    if ($validador->validaDomicilio($candidato_convocatoria->domicilio, 100, 1) == true) {
    } else {
        $num_errores++;
    }

    if ($validador->validaTlf($candidato_convocatoria->tlf) == true) {
    } else {
        $num_errores++;
    }
